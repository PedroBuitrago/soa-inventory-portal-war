...
import co.com.qabox.util.date.DateUtil;
...
public class InventoryAuthApi {
	...
	public MsgMasterDTO getMenu(@QueryParam("user") String clientId, @QueryParam("user") String user, @QueryParam("pass")String pass){
    	...
    	try {
			header.setDate(DateUtil.getFormatDate(DateUtil.dateFormatyyyyMMddhh_mm_ss));
		} catch (ParseException e1) {
			header.setDate("Undefined");
		}
    	try {
			header.setId("IDTXN_" + DateUtil.getFormatDate(DateUtil.dateFormatyyyyMMddhhmmss));
		} catch (ParseException e1) {
